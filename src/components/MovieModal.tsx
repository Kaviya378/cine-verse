            ...
            <div className="md:col-span-7 p-8 md:p-12 flex flex-col justify-center space-y-8 relative z-30">
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap items-center gap-4 text-[10px] font-black uppercase tracking-[0.25em] text-brand-muted">
                  ...
                  <span className="text-white px-3 py-1 bg-white/5 border border-white/10 rounded-md">{movie.language}</span>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <p className="text-brand-gray text-xs md:text-sm leading-relaxed font-sans font-medium">{movie.description}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-8 border-y border-white/5 py-6">
                ...
                <div>
                  ...
                  <p className="text-white text-xs font-black uppercase tracking-wider leading-relaxed">{movie.actors.join(', ')}</p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex flex-wrap gap-2">
                  {movie.genres.map((genre) => (
                    <span key={genre} className="px-4 py-2 bg-white/5 text-white text-[9px] font-black uppercase tracking-widest rounded-lg border border-white/5">
                      {genre}
                    </span>
                  ))}
                  {movie.themes.map((theme) => (
                    <span key={theme} className="px-4 py-2 bg-brand-accent/10 text-brand-accent text-[9px] font-black uppercase tracking-widest rounded-lg border border-brand-accent/20">#{theme}</span>
                  ))}
                </div>
              </div>
            </div>
